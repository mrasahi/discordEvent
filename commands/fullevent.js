    ...
    execute(message, args) {
        DiscordServer.findById(message.guild.id)
            .then(result => {
                ...
                let total = `Total Events: ${wam.count + structure.count + speed.count + links.count + giant.count}\n==============================\n`

                // Message Events. Message will split if over 2000
                message.channel.send(total + wam.data + structure.data + speed.data + links.data + giant.data, { split: true } )
